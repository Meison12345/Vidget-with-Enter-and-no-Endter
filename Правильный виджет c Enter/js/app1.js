...
const $counterLength = $('#counterLength-active');
const maxLength = 400;
let focusElement = $('');
...
$textInsert.on('keydown', function(param) {
    ...
    if (param.key === '[') {
        ...
        if (caretLeftText !== '' && caretRightText !== '' && prevFocusedElem === focusBlock) {
            ...
            // if (regInp.test(caretRightText) === false && regInp.test(caretLeftText) === false && $('.li-elem').length >= 1) {
        ...
        } else if (caretRightText === '' && caretLeftText !== '') {
    ...
    //Вычисление length параграфа с текстом
    setTimeout(() => {
        $counterLength.text($('.text-field').text().length);
    }, 0);
    if ($('.text-field').text().length >= maxLength) {
        if (param.key == 'Backspace' || param.key == 'Delete' || param.key == 'Enter') {} else {
            param.preventDefault();
        }
    }
    ...
    if (param.key === 'Enter') {
        ...
        const maxHeight = +window.getComputedStyle($textInsert[0])['height'].slice(0, -2);
        if (maxHeight >= +$textInsert.css('max-height').slice(0, -2) - 300) {
            param.preventDefault();
        } else {
...
$dropMenuNav.on('click', '.dropMenu-elem', function(e) {
    if ($textInsert.text().length > maxLength || $textInsert.text().length + $(e.target).text().length > maxLength) {
        e.preventDefault();
        return 1;
    } else if ($textInsert.text().length < maxLength) {
        setTimeout(() => {
            $counterLength.text($('.text-field').text().length);
        }, 0);
    }
    // if ($textInsert.text().length + $(e.target).text().length > maxLength) {
